(ns gorillalabs.eventsourcing.mongo-test
  (:require [clojure.test :refer :all]
            [juxt.iota :as i]
            [gorillalabs.eventsourcing.core :refer :all]
            [gorillalabs.eventsourcing.mongo :as mongo]
...
(let [create-id (fn [] (tc/to-long (time/now)))
  ...
  (deflistener [event aggregate :types :deactivated :attach-to listeners]
               (assoc aggregate :status "deactivated"))
  (deflistener [event aggregate :types :activated :attach-to listeners]
  ...
  (with-redefs-fn
    {#'mg/insert!          (fn [coll obj & {:as opts}]
     ...
     #'mongo/version-from* (fn [coll id]
                             ...
                             (is (= id uid))
                             3)}
    #(i/given (with-listeners listeners
                              (mongo/store-events-into* :test 3 (list (new-event :deactivated uid {:reason "Just 4 fun."}))))
              [0 :_v] := 4
              [0 :uid] := uid
              ; [0 :_d] := anything
              [0 :_t] := :deactivated
